<template>
  <div class="white-bg-wrapper">
    ...
    <a-alert
      ...
      message="若小于此时间，薪酬以及考勤计算会异常或者不予计算。"
      type="info"
      show-icon
    />
    <div class="white-bg">
      ...
      >
        <template slot="employmentType" slot-scope="text, record">
          <span v-if="text == 1">小时工</span>
          <span v-if="text == 2">天工</span>
          <span v-if="text == 3">月工</span>
          <span v-if="text == 4">人头费</span>
        </template>
        <template slot="workDayHour" slot-scope="text, record">
          <span v-if="record.employmentType == 1">{{ text }} 小时</span>
          <span v-if="record.employmentType == 2">{{ text }} 小时</span>
          <span v-if="record.employmentType == 3">{{ text }} 天</span>
          <span v-if="record.employmentType == 4">{{ text }} 元/人</span>
        </template>
...
const columns = [
  {
    ...
    key: "employmentType",
    scopedSlots: { customRender: "employmentType" },
    type: "select",
    dataList: [
      {
        name: "小时工",
  ...
  {
    title: "最小工作数",
    dataIndex: "workDayHour",
    key: "workDayHour",
    type: "hide",
    scopedSlots: { customRender: "workDayHour" },
  },
  {
    title: "操作",
    key: "action",
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
      this.$axios
        .get("/workDaySetController/getAdvancedQuery", {
